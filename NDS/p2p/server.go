...
func (srv *Server) setupLocalNode() error {
	...
	NDSdb, _ := enode.OpenDB("/temp/nodes")
	enode.SetNDSDatabase(NDSdb)
...
func (srv *Server) run(dialstate dialer) {
	...
	fmt.Println("Hmmmm. : ", srv.localnode.Node())
	defer srv.loopWG.Done()
	...
	for {
		...
		case c := <-srv.addpeer:
			...
			if err == nil {
				...
				go srv.runPeer(p)
				peers[c.node.ID()] = p
				fmt.Println("added peer n.ID() : ", c.node.ID())
				if p.Inbound() {
					inboundCount++
				}
			}
