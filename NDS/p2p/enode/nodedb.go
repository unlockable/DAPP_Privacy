...
func newNDSDB() (*DB, error) {
	db, err := leveldb.OpenFile("NDSInfo", nil)
	if err != nil {
		return nil, err
	}
	return &DB{lvl: db, quit: make(chan struct{})}, nil
...
// UpdateLastPingReceived updates the last time we tried contacting a remote node.
func (db *DB) UpdateLastPangReceived(key []byte, n int64) error {
	blob := make([]byte, binary.MaxVarintLen64)
	blob = blob[:binary.PutVarint(blob, n)]
	return db.lvl.Put(key, blob, nil)
}
...
// storeInt64 stores an integer in the given key.
func (db *DB) saveNDSDB(key []byte, n int64) error {
	blob := make([]byte, binary.MaxVarintLen64)
	blob = blob[:binary.PutVarint(blob, n)]
	return db.lvl.Put(key, blob, nil)
}
