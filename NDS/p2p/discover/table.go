...
const (
	...
	seedMinTableTime   = 1 //5 * time.Minute
	seedCount          = 30
...
func newTable(t transport, db *enode.DB, NDSdb *enode.DB, bootnodes []*enode.Node, log log.Logger) (*Table, error) {
	tab := &Table{
		net:        t,
		db:         db,
		NDSdb:      NDSdb,
	...
	for _, b := range &tab.buckets {
		for _, n := range b.entries {
			if n.livenessChecks > 0 && now.Sub(n.addedAt) >= seedMinTableTime {
				fmt.Println("Updated Nodes: ", unwrapNode(n))
