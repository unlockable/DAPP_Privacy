...
func ListenV4(c UDPConn, ln *enode.LocalNode, cfg Config) (*UDPv4, error) {
	...
	tab, err := newTable(t, ln.Database(), ln.NDSDatabase(), cfg.Bootnodes, t.log)
	if err != nil {
		return nil, err
	}
	t.tab = tab
...
func (t *UDPv4) lookupWorker(n *node, targetKey encPubkey, reply chan<- []*node) {
	...
	if err == errClosed {
		...
		fmt.Println("########## Successfully connected:", n)
...
func (t *UDPv4) findnode(toid enode.ID, toaddr *net.UDPAddr, target encPubkey) ([]*node, error) {
	...
	rm := t.pending(toid, toaddr.IP, p_neighborsV4, func(r interface{}) (matched bool, requestDone bool) {
		...
		for _, rn := range reply.Nodes {
			...
			//fmt.Println("#############################node : ", n)
			//NDSdb.Put([]byte(fmt.Sprintln(n.ID())), []byte(fmt.Sprintln(n)), nil)
			_ = t.tab.NDSdb.UpdateLastPangReceived([]byte(fmt.Sprintln(n)), 1)
			//t.tab.NDSdb.saveNDSDB([]byte(fmt.Sprintln(n)))
			if err != nil {
